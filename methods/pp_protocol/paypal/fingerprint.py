    ...
    *,
    ...
    p1_data = {
        ...
        "fp2": {
            "browser": {
                "ua": USER_AGENT,
                "lang": (getattr(session.state, "locale", None) or "en-GB").replace("_", "-"),
                "colorDepth": SCREEN["colorDepth"],
                "deviceMemory": 32,
                "hardwareConcurrency": 16,
                "screenResolution": [SCREEN["height"], SCREEN["width"]],
                "availableScreenResolution": [SCREEN["availHeight"], SCREEN["availWidth"]],
                "timezoneOffset": 0 if getattr(session.state, "country", "") in {"GB", "UK"} else 180,
                "timezone": "Europe/London" if getattr(session.state, "country", "") in {"GB", "UK"} else "America/Sao_Paulo",
                "sessionStorage": True,
                "localStorage": True,
                "indexedDb": True,
                "openDatabase": True,
