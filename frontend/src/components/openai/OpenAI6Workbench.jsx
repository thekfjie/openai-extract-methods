...
const providers = [
  { value: 'outlookmail', label: 'OutlookMail 默认分组（推荐）' },
  { value: 'hotmail', label: '本地微软四段池' },
  { value: 'tempmail', label: 'TempMail.lol' },
  { value: 'yyds', label: 'YYDS Mail' },
  { value: 'applemail', label: 'Apple Mail / iCloud 号池' },
];
...
export default function OpenAI6Workbench({ notify }) {
  ...
  const refresh = useCallback(async () => {
    try {
      const results = await Promise.all([apiClient.openai6.get('/config'), apiClient.openai6.get('/status')]);
      const cfg = results[0] || {};
      ...
      if (!hydratedRef.current) {
        ...
        setConfig((current) => ({
          ...
          emailProvider: cfg.emailProvider || saved.emailProvider || 'outlookmail',
  ...
  useEffect(() => {
    ...
    const stream = new EventSource('/openai6/api/events');
    const receive = (raw) => {
      try {
  ...
  const save = async (quiet) => {
    const result = await apiClient.openai6.post('/config', {
    ...
    if (!quiet) notify('OpenAI 6 配置已保存', 'success');
  };
  ...
  const testProxy = async () => {
    ...
    setBusy('proxy');
    try {
      const result = await apiClient.openai6.post('/proxy/test', { proxyUrl: config.proxyUrl.trim() });
  ...
  const start = async () => {
    ...
    try {
      await save(true);
      const result = await apiClient.openai6.post('/start', {
  ...
  const stop = async () => {
    setBusy('stop');
    try {
      await apiClient.openai6.post('/stop', {});
  ...
  const download = async (kind) => {
    try {
      const blob = await apiClient.openai6.blob(kind === 'rt' ? '/download-rt' : '/download');
      downloadBlob(blob, kind === 'rt' ? 'openai6-refresh-tokens.zip' : 'openai6-access-tokens.txt');
  ...
  return (
    <div className="openai6-workbench">
      ...
      <GlassPanel variant="strong" className="openai-control-panel openai6-control-panel">
        <div className="openai-panel-heading">
          <div><h2><TerminalSquare size={18} />OpenAI 6 · AT Maker</h2><small>协议注册 · AutoMyAI 指纹 · 默认分组接码</small></div>
        ...
        <div className="openai-control-scroll">
          ...
          <Field label="邮箱来源" hint={config.emailProvider === 'outlookmail' ? '默认分组现有 ' + (mail.selectable || 0) + ' 个可用账号' : config.emailProvider === 'applemail' ? 'Apple Mail 号池现有 ' + ((mail.appleMail && mail.appleMail.selectable) || 0) + ' 个可用账号' : '备用 Provider'} wide>
          ...
          {config.emailProvider === 'applemail' ? (
            <>
              <Field label="Apple / iCloud 账号" hint={config.appleAccountMode === 'specific' ? '已锁定指定 iCloud 账号；本批固定运行 1 次' : '从 Apple Mail 号池顺序选择'} wide>
                <CustomSelect value={config.appleAccountMode === 'specific' ? config.appleAccountEmail : ''} onChange={(value) => setConfig((current) => ({ ...current, appleAccountEmail: value, appleAccountMode: value ? 'specific' : 'auto', threads: value ? 1 : current.threads, totalRounds: value ? 1 : current.totalRounds }))} options={[{ value: '', label: '自动顺序选择（Apple Mail 号池）' }, ...((mail.appleMail && mail.appleMail.accounts) || []).map((account) => ({ value: String(account.email || ''), label: String(account.email || '') }))]} ariaLabel="Apple Mail 账号" />
              </Field>
