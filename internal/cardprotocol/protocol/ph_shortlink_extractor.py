...
CHECKOUT_CONTEXT_URL = "https://chatgpt.com/backend-api/payments/checkout/{processor}/{checkout_id}"
...
def token_account_id(token: str) -> str:
    """Read the non-secret ChatGPT account identifier from a JWT payload."""
    try:
        segment = normalize_access_token(token).split(".", 2)[1]
...
def _proxy_default_scheme(value: str | None) -> str:
    scheme = str(value or "http").strip().lower()
    if scheme == "socks":
        scheme = "socks5h"
    if scheme not in {"http", "https", "socks4", "socks4a", "socks5", "socks5h"}:
        raise ValueError("代理协议不受支持；可用 HTTP、HTTPS、SOCKS4、SOCKS4A、SOCKS5、SOCKS5H")
    return scheme


def normalize_proxy(raw: str, default_scheme: str = "http", force_scheme: bool = False) -> str:
    """
    接受以下格式：
    ...
    if not value:
        return ""
    default_scheme = _proxy_default_scheme(default_scheme)
    ...
    if "://" not in value:
        ...
        if len(parts) >= 4 and parts[1].isdigit():
            ...
            value = f"{default_scheme}://{username}:{password}@{host}:{port}"
        else:
    ...
    if not scheme_match:
        return ""
    scheme = default_scheme if force_scheme else scheme_match.group("scheme").lower()
...
def split_proxy_pool(raw: str, limit: int = 500, default_scheme: str = "http", force_scheme: bool = False) -> list[str]:
    items: list[str] = []
    seen: set[str] = set()
    for part in re.split(r"[\r\n,;]+", str(raw or "")):
        candidate = normalize_proxy(part, default_scheme=default_scheme, force_scheme=force_scheme)
...
def build_session(
    ...
    runtime_session_id = str(chatgpt_session_id or "").strip() or str(uuid.uuid4())
    headers = {
            "User-Agent": str(user_agent or fingerprint["ua"]),
            "Accept": "*/*",
    ...
    runtime_account_id = str(account_id or token_account_id(token)).strip()
    if runtime_account_id:
        headers["ChatGPT-Account-Id"] = runtime_account_id
    headers["OAI-Session-Id"] = runtime_session_id
...
def resolved_checkout_amount(payload: Any) -> tuple[Decimal, str, str]:
    """Read the authoritative payable total from a resolved OAICS context.

    The sanitized local protocol snapshot uses
    checkout_state.total.total.minorUnitsAmount.  Unlike that snapshot, an
    absent value is not silently converted to zero here: unknown must remain
    unknown so the outer amount gate can retry safely.
    """
    if not isinstance(payload, dict):
        return Decimal("0"), "", ""
    checkout_state = payload.get("checkout_state")
    if not isinstance(checkout_state, dict):
        return Decimal("0"), "", ""
    total = checkout_state.get("total")
    if not isinstance(total, dict):
        return Decimal("0"), "", str(checkout_state.get("currency") or "")
    final_total = total.get("total")
    if not isinstance(final_total, dict) or final_total.get("minorUnitsAmount") is None:
        return Decimal("0"), "", str(checkout_state.get("currency") or "")
    try:
        amount = Decimal(str(final_total["minorUnitsAmount"]))
    except InvalidOperation:
        return Decimal("0"), "", str(checkout_state.get("currency") or "")
    return amount, "checkout_state.total.total.minorUnitsAmount", str(
        checkout_state.get("currency") or ""
    )
...
class Mode8Extractor:
    def __init__(
        ...
        *,
        ...
        # Runtime identity is captured from the actual session that made the
        # Checkout request.  It is persisted for continuity only; no synthetic
        # browser telemetry is generated here.
    ...
    def _session(self, proxy: str) -> Any:
        session, fingerprint = build_session(
            ...
            self.config.session_cookies,
            self.config.user_agent,
            self.config.chatgpt_session_id,
        )
        headers = getattr(session, "headers", {}) or {}
        header_device_id = headers.get("oai-device-id")
        header_user_agent = headers.get("User-Agent")
        actual_device_id = str(
            header_device_id if isinstance(header_device_id, str) else self.config.device_id or ""
        ).strip()
        actual_user_agent = str(
            header_user_agent if isinstance(header_user_agent, str) else self.config.user_agent or ""
        ).strip()
        self._runtime_identity = {
            "checkout_device_id": actual_device_id,
            "checkout_user_agent": actual_user_agent,
            "checkout_chatgpt_session_id": str(
                headers.get("OAI-Session-Id")
                or headers.get("oai-session-id")
                or self.config.chatgpt_session_id
                or ""
            ).strip(),
            "fingerprint_profile": str(fingerprint or "").strip(),
        }
        # Keep the same values if a later stage creates another HTTP session.
        if actual_device_id and not self.config.device_id:
            self.config.device_id = actual_device_id
        if actual_user_agent and not self.config.user_agent:
            self.config.user_agent = actual_user_agent
        if self._runtime_identity["checkout_chatgpt_session_id"] and not self.config.chatgpt_session_id:
            self.config.chatgpt_session_id = self._runtime_identity["checkout_chatgpt_session_id"]
    ...
    def _resolve_checkout_context(
        ...
        """Refetch the official Checkout context before accepting a link."""
        url = CHECKOUT_CONTEXT_URL.format(
            processor=processor_entity,
            checkout_id=checkout_id,
        )
        path = f"/backend-api/payments/checkout/{processor_entity}/{checkout_id}"
        response = session.get(
            url,
            headers={
                "Origin": "https://chatgpt.com",
        ...
        if response.status_code >= 400:
            raise Mode8Error(
                f"Checkout context resolve failed：{summarize_http_error(response)}"
            )
        payload = response_json(response)
        resolved_id = str(payload.get("checkout_session_id") or "").strip()
        if resolved_id and resolved_id != checkout_id:
            raise Mode8Error("Checkout context session 与当前短链不一致")
        methods = [
            str(item).lower()
            for item in (payload.get("payment_method_types") or [])
        ]
        if methods and "card" not in methods:
            raise Mode8Error(f"Checkout context 不支持 card：methods={methods[:8]}")
        return payload
    ...
    def run(self, pool1_text: str = "", pool2_text: str = "") -> dict[str, Any]:
        ...
        # Preserve one device identity across stage 1, stage 2 and the final
        # context refetch even when the network egress changes.  This matches
        # the local protocol snapshot's preserve_checkout_identity behavior.
        if not str(self.config.device_id or "").strip():
        ...
        # The create/update response is only provisional.  The local sanitized
        # payment protocol refetches the OAICS context and reads the final due
        # amount from checkout_state; do the same before the amount gate sees
        # this result so a stale list price can never be accepted as final.
        resolved_payload = self._resolve_checkout_context(
            validation_session,
            checkout_id,
            processor_entity,
        )
        amount, amount_source, resolved_currency = resolved_checkout_amount(
            resolved_payload
        )
        if resolved_currency and resolved_currency.upper() != self.config.currency.upper():
            raise Mode8Error(
                f"Checkout context 币种不匹配：{resolved_currency.upper()}，期望 {self.config.currency.upper()}"
            )
        ...
        result = {
            ...
            "url": result_url,
            # These are continuity metadata from the real session.  The portal
            # stores them with the Checkout context so final payment can reuse
            # the same identity instead of falling back to a hard-coded UA or
            # a newly generated device ID.
            "checkout_device_id": self._runtime_identity.get("checkout_device_id") or str(self.config.device_id or ""),
            "checkout_chatgpt_session_id": self._runtime_identity.get("checkout_chatgpt_session_id") or str(self.config.chatgpt_session_id or ""),
            "checkout_user_agent": self._runtime_identity.get("checkout_user_agent") or str(self.config.user_agent or ""),
...
class _FakeSession:
    ...
    def get(self, url: str, **_: Any) -> _FakeResponse:
        ...
        if url == CHECKOUT_CONTEXT_URL.format(
            processor="openai_ie",
...
def run_self_test() -> None:
    ...
    assert result["amount_source"] == (
        "checkout_state.total.total.minorUnitsAmount"
    ), result
    assert result["context_verified"] is True, result
