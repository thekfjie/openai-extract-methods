...
let stopped=false;
let running=false;
let cardConfirmTail=Promise.resolve();
let cardConfirmActive=0;
let proxyCursor=0;
const results=[];
...
async function api(url,options={}){
  ...
  try{
    ...
    const timedOut=cause?.name==='AbortError'&&!externalSignal?.aborted;
    const error=new Error(timedOut?('Request timed out after '+Math.round(timeoutMs/1000)+'s'):(cause?.message||'Network request failed'));
    clearTimeout(timer);error.httpStatus=0;error.networkError=true;error.timedOut=timedOut;throw error;
  }
...
async function mountCardFields(publishableKey){
  ...
  const style={
    base:{fontSize:'18px',fontFamily:'"SF Pro UI",-apple-system,BlinkMacSystemFont,"PingFang SC","Microsoft YaHei",sans-serif',color:lightTheme?'#172019':'#f4f8ff',iconColor:lightTheme?'#587800':'#58d9b2','::placeholder':{color:lightTheme?'#7a857c':'#6f8299'}},
    invalid:{color:'#ff5f72',iconColor:'#ff5f72'}
  };
...
async function resolvePendingKey(session){
  ...
  let lastChangeAt=startedAt,lastSignature='';
  for(let i=0;i<90;i++){
    if(Date.now()-startedAt>180000)throw new Error('Payment context preparation timed out after 180s');
    const data=await apiWithRetry(apiBase+'/api/card-bind/key-probe/'+probeId,{timeoutMs:45000},2);
    const probe=data.probe||{};
    const signature=[probe.status,Number(probe.progress||0),probe.message||''].join('|');
    if(signature!==lastSignature){lastSignature=signature;lastChangeAt=Date.now()}
    if(Date.now()-lastChangeAt>60000)throw new Error('Payment context preparation stalled for 60s');
    status('Preparing payment context: '+Number(probe.progress||0)+'%');
  ...
  throw new Error('Payment context preparation timed out');
}
...
async function prepareSessionOne(token,index,total,existingRow=null){
  ...
  updateRow(row,'Preparing SetupIntent','running');
  let session=prefetchedSessions.get(token)||null;
  if(session)prefetchedSessions.delete(token);
  if(!session){
    session=await apiWithProxyRetry(apiBase+'/api/card-bind/session',{access_token:token,proxy_protocol:selectedProxyProtocol()},3,(attempt,total)=>updateRow(row,'Session proxy retry '+attempt+' / '+total,'running'));
    session=await resolvePendingKey(session);
  }
  const sessionKey=String(session.publishable_key||'');
  if(sessionKey&&sessionKey!==stripeKey){
    throw new Error('This account uses a different Stripe key. Run it in a separate batch after reloading the card fields.');
  }
  ...
  updateRow(row,'SetupIntent ready; waiting for confirm lane','running');
  return{token,index,row,session,account};
}
async function withCardConfirmLock(row,operation){
  let release;
  const previous=cardConfirmTail;
  cardConfirmTail=new Promise(resolve=>{release=resolve});
  if(cardConfirmActive>0)updateRow(row,'Waiting for the secure card confirmation lane','running');
  await previous.catch(()=>{});
  cardConfirmActive++;
  document.body.classList.add('card-confirm-busy');
  try{return await operation()}
  finally{
    cardConfirmActive=Math.max(0,cardConfirmActive-1);
    if(!cardConfirmActive)document.body.classList.remove('card-confirm-busy');
    release();
  }
}
async function confirmPreparedCard(prepared){
  const {row,session}=prepared;
  return await withCardConfirmLock(row,async()=>{
    let confirmation=null,lastError=null;
    for(let attempt=1;attempt<=3;attempt++){
      updateRow(row,attempt===1?'Confirming card in isolated lane':('Confirmation lane recovery '+attempt+' / 3'),'running');
      try{
        ...
        const message=String(confirmation?.error?.message||'');
        if(/in-flight confirmCardSetup/i.test(message)){
        ...
        lastError=error;
        if(!/in-flight confirmCardSetup/i.test(String(error?.message||''))||attempt>=3)throw error;
        await sleep(900*attempt);
      }
    }
    if(!confirmation&&lastError)throw lastError;
    if(confirmation?.error)throw new Error(confirmation.error.message||'Card confirmation failed');
    if(!confirmation?.setupIntent||confirmation.setupIntent.status!=='succeeded')throw new Error('Card setup status: '+(confirmation?.setupIntent?.status||'unknown'));
    const paymentMethodValue=confirmation.setupIntent.payment_method;
    const pm=typeof paymentMethodValue==='string'?paymentMethodValue:paymentMethodValue?.id;
    if(!pm)throw new Error('Stripe did not return a PaymentMethod');
    const cardMeta=(paymentMethodValue&&typeof paymentMethodValue==='object')?(paymentMethodValue.card||{}):{};
    window.dispatchEvent(new CustomEvent('neo-card-confirmed',{detail:{brand:String(cardMeta.brand||''),last4:String(cardMeta.last4||''),expMonth:Number(cardMeta.exp_month||0),expYear:Number(cardMeta.exp_year||0)}}));
    return{...prepared,paymentMethod:pm};
  });
}
async function dispatchPreparedCard(prepared){
  const {row,session,token,account,paymentMethod}=prepared;
  updateRow(row,'Attaching card while next account confirms','running');
  await apiWithProxyRetry(apiBase+'/api/card-bind/default',{payment_method_id:paymentMethod,record_id:session.record_id,access_token:token,proxy_protocol:selectedProxyProtocol()},3,(attempt,total)=>updateRow(row,'Attach proxy retry '+attempt+' / '+total,'running'));
...
async function prepareOne(token,index,total,existingRow=null){
  const prepared=await prepareSessionOne(token,index,total,existingRow);
  const confirmed=await confirmPreparedCard(prepared);
  return await dispatchPreparedCard(confirmed);
}
...
startButton.addEventListener('click',async()=>{
  ...
  const unitPerAccount=linkOnly?2:4;
  const totalUnits=tokens.length*unitPerAccount;
  setStep(linkOnly?1:2);setProgress(0,totalUnits,linkOnly?'\u76f4\u63a5\u63d0\u94fe\u5e76\u53d1 '+concurrency:'\u4f1a\u8bdd\u5e76\u53d1 '+concurrency+' / \u786e\u8ba4\u5355\u901a\u9053 / \u6536\u5c3e\u5e76\u53d1');
  ...
  if(linkOnly){
    let nextSubmit=0;
    const submitWorker=async workerId=>{
      await sleep(workerId*80);
      while(!stopped){
        const index=nextSubmit++;
        if(index>=tokens.length)return;
        try{
          status('\u6b63\u5728\u5e76\u53d1\u63d0\u4ea4\u76f4\u63a5\u63d0\u94fe '+(index+1)+' / '+tokens.length);
          const job=await prepareLinkOnly(tokens[index],index,tokens.length);
          preparedJobs.push(job);finishedUnits++;
          progressUpdate('\u5df2\u63d0\u4ea4 '+preparedJobs.length+' / '+tokens.length+' \u4e2a\u63d0\u94fe\u4efb\u52a1');
        }catch(error){
          failed++;finishedUnits+=2;
          const row=document.querySelector('[data-at-index="'+index+'"]')||taskRows.querySelectorAll('.task-row')[index];
          if(row)updateRow(row,'\u63d0\u94fe\u63d0\u4ea4\u5931\u8d25','failed',error.message);
          progressUpdate('\u4efb\u52a1\u5b8c\u6210 '+(success+failed)+' / '+tokens.length);
        }
      }
    };
    await Promise.all(Array.from({length:Math.min(concurrency,tokens.length)},(_,index)=>submitWorker(index)));
  }else{
    const sessionResults=new Array(tokens.length);
    let nextSession=0;
    const sessionWorker=async workerId=>{
      await sleep(workerId*70);
      while(!stopped){
        const index=nextSession++;
        if(index>=tokens.length)return;
        try{
          status('\u6b63\u5728\u5e76\u53d1\u51c6\u5907 SetupIntent '+(index+1)+' / '+tokens.length+'\uff0c\u5e76\u53d1 '+concurrency);
          sessionResults[index]=await prepareSessionOne(tokens[index],index,tokens.length);
          finishedUnits++;
          progressUpdate('\u4f1a\u8bdd\u5df2\u51c6\u5907 '+sessionResults.filter(Boolean).length+' / '+tokens.length);
        }catch(error){
          failed++;finishedUnits+=unitPerAccount;
          const row=document.querySelector('[data-at-index="'+index+'"]')||taskRows.querySelectorAll('.task-row')[index];
          if(row)updateRow(row,'SetupIntent \u51c6\u5907\u5931\u8d25','failed',error.message);
          progressUpdate('\u4efb\u52a1\u5b8c\u6210 '+(success+failed)+' / '+tokens.length);
        }
      }
    };
    await Promise.all(Array.from({length:Math.min(concurrency,tokens.length)},(_,index)=>sessionWorker(index)));

    const dispatchActive=new Set();
    const scheduleDispatch=async confirmed=>{
      while(dispatchActive.size>=concurrency)await Promise.race(dispatchActive);
      let promise;
      promise=dispatchPreparedCard(confirmed).then(job=>{
        preparedJobs.push(job);finishedUnits++;
        progressUpdate('\u7ed1\u5361\u6536\u5c3e '+preparedJobs.length+' / '+tokens.length+'\uff0cCheckout \u5df2\u63d0\u4ea4');
      }).catch(error=>{
        failed++;finishedUnits+=2;
        updateRow(confirmed.row,'\u7ed1\u5361\u6536\u5c3e\u5931\u8d25','failed',error.message);
        progressUpdate('\u4efb\u52a1\u5b8c\u6210 '+(success+failed)+' / '+tokens.length);
      }).finally(()=>dispatchActive.delete(promise));
      dispatchActive.add(promise);
    };

    for(let index=0;index<sessionResults.length&&!stopped;index++){
      const prepared=sessionResults[index];
      if(!prepared)continue;
      status('\u5b89\u5168\u786e\u8ba4\u901a\u9053 '+(index+1)+' / '+tokens.length+'\uff1b\u5df2\u786e\u8ba4\u7684\u8d26\u53f7\u5728\u540e\u53f0\u5e76\u53d1\u7ed1\u5b9a');
      try{
        const confirmed=await confirmPreparedCard(prepared);
        finishedUnits++;
        progressUpdate('\u5361\u7247\u786e\u8ba4\u5b8c\u6210 '+(index+1)+' / '+tokens.length+'\uff0c\u5e76\u53d1\u6536\u5c3e\u4e2d');
        await scheduleDispatch(confirmed);
      }catch(error){
        failed++;finishedUnits+=3;
        updateRow(prepared.row,'\u5361\u7247\u786e\u8ba4\u5931\u8d25','failed',error.message);
        progressUpdate('\u4efb\u52a1\u5b8c\u6210 '+(success+failed)+' / '+tokens.length);
      }
    }
    await Promise.all([...dispatchActive]);
  }
  ...
  setStep(3);setProgress(totalUnits,totalUnits,'\u6279\u91cf\u4efb\u52a1\u5b8c\u6210');
...
window.addEventListener('neo-theme-changed',async()=>{
  if(!cardFieldsMounted||running||cardConfirmActive||!stripeKey)return;
  try{await mountCardFields(stripeKey);cardReady.textContent='Theme changed. Please re-enter the card details.';cardReady.className='inline-state'}catch{}
});
