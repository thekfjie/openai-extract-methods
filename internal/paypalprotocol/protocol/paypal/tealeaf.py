...
def send_tealeaf_data(session, page_url: str, dom_html: Optional[str] = None,
    ...
    if not mouse_data:
        ...
        mouse_data = generate_mouse_path(
            random.randint(80, max(80, int(viewport["width"] * 0.35))),
            random.randint(80, max(80, int(viewport["height"] * 0.35))),
            random.randint(max(100, int(viewport["width"] * 0.35)), max(120, int(viewport["width"] * 0.8))),
            random.randint(max(100, int(viewport["height"] * 0.35)), max(120, int(viewport["height"] * 0.75))),
        )
