...
from paypal.session import PayPalAuthChallengeError, PayPalSession, sanitize_for_log
...
class PayPalFlow:
    def __init__(
        ...
        self.state = SessionState(
            ...
            lang=resolved_lang,
            fingerprint_profile=build_task_fingerprint(
                resolved_country,
                resolved_locale,
                seed=uuid.uuid4().hex,
            ),
        ...
        logger.info(
            "Task fingerprint profile={} country={} locale={} screen={}x{} memory={} concurrency={}",
            self.state.fingerprint_profile.get("profileId", "<missing>"),
            resolved_country,
            resolved_locale,
            self.state.fingerprint_profile.get("screen", {}).get("width"),
            self.state.fingerprint_profile.get("screen", {}).get("height"),
            self.state.fingerprint_profile.get("deviceMemory"),
            self.state.fingerprint_profile.get("hardwareConcurrency"),
        )
    ...
    def _phase0_initial_load(self):
        ...
        if resp.status_code == 403:
            logger.error("Initial agreement page returned a DataDome/browser challenge")
            raise PayPalAuthChallengeError(
                operation="InitialAgreementPage",
                status=resp.status_code,
                paypal_debug_id=str(resp.headers.get("paypal-debug-id") or ""),
                page_family="datadome",
                challenge_kind="datadome",
                form_action=str(resp.url),
                challenge_form={
                    "formAction": str(resp.url),
                    "sessionId": "",
                    "csrfPresent": False,
                    "requestIdPresent": False,
                    "hashPresent": False,
                    "recaptchaSiteKey": "",
                    "captchaIframePresent": "captcha" in resp.text.lower(),
                },
            )
        ...
        self._capture_ec_token(str(resp.url), html, source="initial agreement page")
    ...
    def _assert_identity_uplift(self, stage: str = "pre-authorize") -> None:
        ...
        logger.info(
            ...
            stage,
            self.state.user_id,
            sanitize_for_log({"token": self.state.ec_token})["token"],
        )
    ...
    @staticmethod
    def _extract_ec_token(*sources: str) -> str:
        """Extract the EC checkout token from URLs, HTML, or escaped Next data."""
        patterns = (
            r"(?:[?&]|&amp;)token=(EC-[A-Z0-9]+)",
            r'["\\]ecToken["\\]\s*:\s*["\\](EC-[A-Z0-9]+)',
            r"\b(EC-[A-Z0-9]+)\b",
        )
    ...
    def _capture_ec_token(self, *sources: str, source: str = "response") -> str:
        token = self._extract_ec_token(*sources)
        if token:
            self.state.ec_token = token
            logger.info(
                "EC Token ({}): {}",
                source,
                sanitize_for_log({"ec_token": token})["ec_token"],
            )
        return token

    def _require_ec_token(self, stage: str) -> str:
        token = str(self.state.ec_token or "").strip()
        if not token.startswith("EC-"):
            self._emit(
                "account",
                "未建立 EC checkout 上下文，已停止进入短信与注册阶段",
                ecTokenPresent=False,
                contextTokenType="missing",
            )
            raise RuntimeError(
                f"EC checkout token missing at {stage}; BA token must not be used for OTP or signup"
            )
        return token
    ...
    def _extract_modxo_action_ids(self, html: str, base_url: str):
        ...
        def scan(text: str) -> bool:
            changed = False
            # Current PayPal bundles declare both references in one compact
            # module. Parse each call independently so the first 40/64-byte
            # id cannot bleed into the following action name.
            for match in re.finditer(
                r'createServerReference\(\s*"([0-9a-f]{32,64})"[^)]{0,240}?"(showCreateAccountAction|createUserAction)"\s*\)',
                text or "",
                re.I,
            ):
                action_id, action_name = match.groups()
                attr = next((key for key, value in action_names.items() if value.lower() == action_name.lower()), "")
                if attr and not getattr(self.state, attr):
            ...
            for attr, action_name in action_names.items():
                ...
                action_id = ""
                direct_patterns = (
                    rf'createServerReference\(\s*"([0-9a-f]{{32,64}})"[^)]{{0,240}}?"{re.escape(action_name)}"\s*\)',
                    rf'"([0-9a-f]{{32,64}})"[^;\n]{{0,240}}?"{re.escape(action_name)}"',
    ...
    def _confirm_phone_with_retry(self, token: str, signup_url: str):
        ...
        while True:
            ...
            except PayPalAuthChallengeError as challenge:
                # An HTML authchallenge response here means PayPal blocked the
                # initiate request before an SMS challenge was created. Do not
                # mislabel this as a carrier/phone failure or burn more numbers.
                details = {
                    "challengeKind": challenge.challenge_kind,
                    "pageFamily": challenge.page_family,
                    "httpStatus": challenge.status,
                    "paypalDebugId": challenge.paypal_debug_id or "",
                    "manualActionRequired": True,
                    "smsCreated": False,
                    "form": dict(challenge.challenge_form or {}),
                }
                self._emit(
                    "verification",
                    "PayPal 身份检查阻止了短信发送；请先完成浏览器检查后再继续",
                    challenge=details,
                    smsCreated=False,
                )
                logger.warning(
                    "OTP initiate stopped by PayPal auth challenge: page_family={} "
                    "challenge_kind={} paypal_debug_id={}",
                    challenge.page_family,
                    challenge.challenge_kind,
                    challenge.paypal_debug_id or "<missing>",
                )
                raise
            except Exception as e:
    ...
    def _build_signup_variables(self, token: str) -> dict:
        ...
        # The successful GB HAR omits `state` entirely. Sending a city/county
        # value here changes PayPal's account-candidate validation behavior.
        if self.address.state and country != "GB":
    ...
    def _send_signup_attempt(self, token: str, signup_url: str) -> dict:
        ...
        signup_result = self.session.graphql(
            ...
            extra_body={
                "fn_sync_data": build_signup_fn_sync_data(
                    token,
                    profile=self.state.fingerprint_profile,
                )
            },
        )
        logger.info(
    ...
    def _signup_with_card_retry(self, token: str, signup_url: str):
        ...
        for attempt in range(1, self.max_card_attempts + 1):
            ...
            try:
                ...
                self._emit(
                    "account",
                    "PayPal 返回身份/风控挑战，需要在浏览器完成检查后重新开始账户流程",
                    challenge=details,
                )
                logger.warning(
                    "Signup stopped at PayPal auth challenge: page_family={} challenge_kind={} paypal_debug_id={}",
                    challenge.page_family,
                    challenge.challenge_kind,
                    challenge.paypal_debug_id or "<missing>",
                )
    ...
    def _phase2_create_account(self):
        ...
        if resp is None:
            if self.state.ec_token:
                # The successful UK capture performs this approval handoff after
                # the ModXO create-user action and receives checkoutweb/signup.
                # When the dynamic bundle cannot be inspected but the initial
                # agreement page already exposed its EC context, replay that
                # canonical handoff instead of downgrading later calls to BA.
                params = [
                ...
                handoff_url = "https://www.paypal.com/agreements/approve?" + urllib.parse.urlencode(params)
                logger.info("Using HAR-aligned EC approval handoff")
                resp = self.session.get(
                    handoff_url,
                    headers={
                        "Accept": "text/html,application/xhtml+xml,application/xml;q=0.9,*/*;q=0.8",
                ...
                # Compatibility probe for older deployments. Its response may
                # reveal EC, but it is never allowed to drive OTP/signup as BA.
                base_url = (
                    f"https://www.paypal.com/pay?ssrt={self.state.ssrt}"
        ...
        while resp.status_code in (301, 302, 303, 307, 308):
            ...
            if not redirect_url:
                break
            redirect_url = urllib.parse.urljoin(str(resp.url), redirect_url)
        ...
        self._capture_ec_token(str(resp.url), html, source="create-account response")
        self._require_ec_token("post-create-account")
        self._emit(
            "account",
            "EC checkout 上下文已建立，准备进入身份提升",
            ecTokenPresent=True,
            contextTokenType="EC",
        )
    ...
    def _phase3_signup_and_2fa(self):
        ...
        token = self._require_ec_token("pre-otp")
        ...
        self._emit("account", "正在创建 Member 账户并完成身份提升")
    ...
    def _phase4_authorize(self) -> dict:
        ...
        billing_agreement_id = self._require_ec_token("pre-authorize")
        logger.info(
