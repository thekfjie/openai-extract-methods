...
def send_analytics_ts(session, page_name: str, ba_token: str,
                      ...
                      event: str = "im", country: str = "BR",
                      locale: str = "pt_BR"):
    """Send PayPal Analytics tracking pixel (t.paypal.com/ts)."""
    ts = int(time.time() * 1000)
    country = (country or "BR").upper()
    locale = (locale or "pt_BR").replace("-", "_")
    # Rough timezone offsets in minutes for analytics only.
    tz_offsets = {"BR": -180, "GB": 0, "UK": 0, "US": 300, "TH": -420}
    profile = getattr(getattr(session, "state", None), "fingerprint_profile", {}) or {}
    screen = profile.get("screen") or SCREEN
    viewport = profile.get("viewport") or VIEWPORT
    params = {
        "v": "1.15.0",
        "t": str(ts),
        "g": str(tz_offsets.get(country, 0)),
        ...
        "rsta": locale,
        "ccpg": country if country != "UK" else "GB",
        "cnac": country if country != "UK" else "GB",
        "flnm": "Hagrid",
        "e": event,
        "fpti_sdk_name": "pa-js",
