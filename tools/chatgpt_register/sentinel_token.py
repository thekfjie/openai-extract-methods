...
from integrations.oai_fingerprint import (
    fingerprint_browser_date,
    fingerprint_field as _fingerprint_field,
    fingerprint_languages as _fingerprint_languages,
    sentinel_navigator_value,
)
...
def gather_fingerprint_data(sid: str, fingerprint: Mapping[str, Any] | None = None) -> list:
    """
    收集浏览器指纹数据 (25项), 与 SDK initializeAndGatherData() 一致。
    索引 [3] 和 [9] 会被 PoW 覆写。
    """
    now_str = fingerprint_browser_date(fingerprint)
    ...
    nav_val = sentinel_navigator_value(nav_prop, fingerprint, fallback_user_agent=FAKE_USER_AGENT)
    screen_width = int(_fingerprint_field(fingerprint, "screen_width", 1920))
    screen_height = int(_fingerprint_field(fingerprint, "screen_height", 1080))
    user_agent = str(_fingerprint_field(fingerprint, "user_agent", FAKE_USER_AGENT))
    language = str(_fingerprint_field(fingerprint, "lang", "en-US"))
    languages = _fingerprint_languages(fingerprint)
    hardware_concurrency = int(_fingerprint_field(fingerprint, "hardware_concurrency", 8))
    ...
    return [
        ...
        ",".join(str(item) for item in languages), # [8] navigator.languages.join(",")
        ...
        hardware_concurrency,                   # [16] navigator.hardwareConcurrency
...
def generate_requirements_token(sid: str, fingerprint: Mapping[str, Any] | None = None) -> str:
    """
    生成 requirements token (内部 seed, difficulty "0")
    返回: "gAAAAAC" + PoW answer
    """
    seed = str(random.random())  # this.requirementsSeed
    data = gather_fingerprint_data(sid, fingerprint)
    answer = solve_pow(seed, "0", data)
    return "gAAAAAC" + answer


def generate_enforcement_token(chat_req: dict, sid: str, fingerprint: Mapping[str, Any] | None = None) -> str:
    """
...
def _run_vm_via_node(chat_req: dict, xor_key: str, vm_type: str, flow: str = "oauth_create_account") -> Optional[str]:
    ...
    script_dir = os.path.join(module_dir, "sentinel_vm")
    gen_script = os.path.join(script_dir, "gen_token_jsdom.js")
    if not os.path.exists(gen_script):
        script_dir = module_dir
        gen_script = os.path.join(script_dir, "gen_token_jsdom.js")
    sdk_path = os.path.join(os.path.expanduser("~"), ".codeium", "windsurf", "sentinel_sdk_full.js")
    if not os.path.exists(gen_script) or not os.path.exists(sdk_path):
        return None
...
class SentinelTokenProvider:
    ...
    def __init__(self, impersonate: str = "firefox144", cookies: dict = None, fingerprint: Mapping[str, Any] | None = None):
        self.impersonate = impersonate
        self.fingerprint = dict(fingerprint) if isinstance(fingerprint, Mapping) else None
    ...
    async def _init(self, flow: str, device_id: str) -> None:
        ...
        proof = await asyncio.to_thread(generate_requirements_token, self.sid, self.fingerprint)
    ...
    async def get_token(self, flow: str, device_id: str) -> Optional[dict]:
        ...
        p_token = await asyncio.to_thread(generate_enforcement_token, chat_req, self.sid, self.fingerprint)
    ...
    async def get_so_token(self, flow: str, device_id: str) -> Optional[dict]:
        ...
        if not self._cached_chat_req or self._cached_flow != flow or self._device_id != device_id:
            await self._init(flow, device_id)
