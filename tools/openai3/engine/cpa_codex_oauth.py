...
class MailClient:
    ...
    async def poll_verify_code(self, email: str, timeout: int = 120, interval: int = 5, since: float = None) -> Optional[str]:
        """轮询验证码, since 为 Unix 时间戳, 只接受该时间之后的邮件"""
        s = await self._get_session()
        from datetime import datetime, timezone
        for i in range(timeout // interval):
            r = await s.get(f"{MAIL_BASE}/api/verify-code", params={"email": email, "keyword": "openai"})
            try:
                d = r.json()
            except Exception:
