...
// Support reverse-proxy prefix (e.g. /openai2) without mixing with main automyai.
const API_BASE = (() => {
  const p = window.location.pathname || "/";
  if (p === "/openai2" || p.startsWith("/openai2/")) return "/openai2";
  return "";
})();
...
function streamRun(runId) {
  ...
  const es = new EventSource(withBase(`/api/runs/${runId}/stream`));
  currentEs = es;
...
$("#credExportAuth").addEventListener("click", async (e) => {
  ...
  if (!email) return;
  try {
    const resp = await fetch(withBase(`/api/registered/${encodeURIComponent(email)}/auth_json`));
    if (!resp.ok) {
...
function _connectAutoStream() {
  ...
  const es = new EventSource(withBase("/api/auto/stream"));
  _autoEs = es;
